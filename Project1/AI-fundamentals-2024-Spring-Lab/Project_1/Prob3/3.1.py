...
class MazeSolver:
    ...
    def solve_maze_bfs(self):
        shortest_path = []
        while self.queue:
            x, y, moves, path = self.queue.popleft()
            if x == self.n - 1 and y == self.m - 1:
                path += [(x, y)]
                if not shortest_path or len(path) < len(shortest_path[0][0]):
            ...
            for dx, dy in self.directions:
                ...
                if 0 <= nx < self.n and 0 <= ny < self.m and (self.maze[nx][ny] == 0 or self.maze[nx][ny] == 2) and (nx, ny) not in self.visited:
                    ...
                    if self.maze[nx][ny] == 2:
                        self.queue.append((nx, ny, moves + 2, new_path))
                    else:
                        self.queue.append((nx, ny, moves + 1, new_path))
